    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${APIpath}courses/created-by/${user.userId}`,
        );
  ...
  const handleSaveImageUrl = async () => {
    ...
    try {
      const response = await fetch(`${APIpath}users/${user.userId}`, {
        ...
        body: JSON.stringify({ email: "", name: "", avt: uploadedImageUrl }),
      });
    ...
    return (
      <div className="page">
        <div
          ...
          className={`col ${isEditing ? "blur-background" : ""}`}
        >
          ...
          <div id="user-info">
            {user.username}
            <div style={{ fontSize: "20px" }}>{myCourse.length} Khóa học</div>
          </div>
          <div
            id="edit-info-btn"
            style={{ right: 40 }}
        ...
        <div className={`${isEditing ? "blur-background" : ""}`}>
          ...
          <CoursePreviewList courses={randomCourses} />
        </div>
        {isEditing ? (

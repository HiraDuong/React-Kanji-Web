...
function CourseProgress() {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${APIpath}userProgress/Remember/uc/${user.userId}/${courseId}`,
          {
            timeout: 5000,
          },
        );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `${APIpath}userProgress/NotRemember/uc/${user.userId}/${courseId}`,
          {
            timeout: 5000,
          },
        );
    ...
    return (
      <div className="page">
        <div id="header-container">
          ...
          <div id="info-container">
            <div id="description-container">
              ...
              <div className="progress-course-description">{formattedDate}</div>
          ...
          <div id="progress-btn-container">
            {user.userId === course.created_by_user_id ? (
              <Link
                ...
                to={`/course/update?courseId=${courseId}`}
                onClick={() => {}}
              >
                Chỉnh sửa
              </Link>
        ...
        <div className="progress-word-container" id="remember-container">
          <div className="progress-title">Từ đã nhớ</div>
          <div>
            {remember.map((remember) => (
              <ProgressWord word={remember} key={remember.kanji} />
            ))}
          </div>
        </div>
        <div className="progress-word-container" id="forgot-container">
          <div style={{ color: "#f7821b" }} className="progress-title">
            Từ chưa nhớ
          </div>
          <div>
            {notRemember.map((notRemember) => (
              <ProgressWord word={notRemember} key={notRemember.kanji} />
            ))}
          </div>
        </div>
      </div>

  ...
  const handleCancelUploadImgUrl = ()=>{
    ...
    const handleSaveWord = async()=>{
        ...
        try {
            ...
            alert(`Tạo từ mới thành công: ${wordData.kanji}, ${wordData.meaning} , ${wordData.pronounce}`)
          } catch (error) {
            alert ("Từ đã tồn tại !")
    ...
    if(user.role === 1) return<PageNotFound/>
    else
    return(
        <div className="page" >
       ...
       >
        ...
        <div className="word-image-container" onClick={()=>{
          handleAvtClick()
        }}>
            
        <img src= {uploadedImageUrl || "/image/default_img.png"}/>
        Tải ảnh lên
        </div>
